...
export class WelcomeComponent implements OnInit {
  ...
  ShortenURLWithRetry(): Observable<any> {
    ...
    return this.http.post('./api/save', JSON.stringify(data), {
      headers: {
  ...
  ShortenURL(): void {
    if (this.myurl.indexOf(window.location.host) > 0) {
      ...
      if (this.myurl.slice(-1) != '$') {
        ...
        this.http.post('./api/getdata', JSON.stringify(data), {
          headers: {
            "Content-Type": "application/json"
          }
        }).subscribe(res => {
    ...
    if (this.status && document.getElementById('shortenurl').innerHTML == 'MINIFY') {
      ...
      shorten.subscribe(res => {
        this.myurl = window.location.protocol + "//" + window.location.host + '/' + res['code'];
